...
map_date_ = dt.datetime(2014, 12, 4, 10, 35)

f ='./files/dmsp/rfe39_PS.APL_V0116S024CB0005_SC.U_DI.A_GP.F18-SSUSI_PA.APL-SDR-DISK_DD.20141203_SN.26431-00_DF.NC' #my file
#f ='./files/dmsp/rfe39_PS.APL_V0116S024CB0005_SC.U_DI.A_GP.F18-SSUSI_PA.APL-SDR-DISK_DD.20141203_SN.26430-00_DF.NC' #my file
...
title = 'RFE:39 DMSP F18 and SuperDARN inv 03.12.2014 15:50'
...
sc_time = [dt.datetime(2014, 12, 3) + dt.timedelta(seconds=x) for x in fh.variables['DMSP_COORDS_TIME'].data.ravel()]
...
##NOAA-19
lat2,lon2,time2=satorbit('./files/orbits/39_NOAA-19.txt')
...
for i, time in enumerate(time2):
    if time.minute % 5 == 0 and time.second == 0:
        ...
        if map_.llcrnrx < x < map_.urcrnrx and map_.llcrnry < y < map_.urcrnry:
            ...
            ax.text(x, y, 'NOAA-19 '+'{:%H:%M}'.format(time), color='k', zorder=7, fontsize=3, fontweight='bold',
                          path_effects=[pe.withStroke(foreground='w', linewidth=2)])
...
#Add SuperDARN Scan
#===================================================
sTime=dt.datetime(2014, 12, 3, 15, 50)
...
plt.savefig('./dmsp39_nosd.png', dpi=600)
