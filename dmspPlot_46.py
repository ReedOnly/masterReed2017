...
map_date_ = dt.datetime(2014, 12, 11, 10, 19)

f ='./files/dmsp/rfe46_PS.APL_V0116S024CB0005_SC.U_DI.A_GP.F18-SSUSI_PA.APL-SDR-DISK_DD.20141211_SN.26541-00_DF.NC' #my file
#f ='./files/dmsp/rfe46_PS.APL_V0116S024CB0005_SC.U_DI.A_GP.F18-SSUSI_PA.APL-SDR-DISK_DD.20141211_SN.26540-00_DF.NC' #my file
...
title = 'RFE:46 DMSP F16 and SuperDARN 11.12.2014 10:19'
...
sc_time = [dt.datetime(2014, 12, 11) + dt.timedelta(seconds=x) for x in fh.variables['DMSP_COORDS_TIME'].data.ravel()]
...
##NOAA-15
lat2,lon2,time2=satorbit('./files/orbits/46_NOAA-15.txt')
...
for i, time in enumerate(time2):
    if time.minute % 5 == 0 and time.second == 0:
        ...
        if map_.llcrnrx < x < map_.urcrnrx and map_.llcrnry < y < map_.urcrnry:
            ...
            ax.text(x, y, 'NOAA-15 '+'{:%H:%M}'.format(time), color='k', zorder=7, fontsize=3, fontweight='bold',
                          path_effects=[pe.withStroke(foreground='w', linewidth=2)])
...
#Add SuperDARN Scan
#===================================================
sTime=dt.datetime(2014, 12, 11, 10, 19)
data = pydarn.sdio.radDataRead.radDataOpen(sTime, 'rkn', eTime=sTime+dt.timedelta(seconds=60))
...
plt.savefig('./dmsp46.png', dpi=600)
