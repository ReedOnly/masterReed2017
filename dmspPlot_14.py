...
map_date_ = dt.datetime(2014, 12, 01, 16, 8)

f ='./files/dmsp/rfe14_PS.APL_V0116S024CB0005_SC.U_DI.A_GP.F18-SSUSI_PA.APL-SDR-DISK_DD.20141201_SN.26403-00_DF.NC' #my file
...
title = 'DMSP F18 and SuperDARN 01.12.2014 16:08'
...
sc_time = [dt.datetime(2014, 12, 01) + dt.timedelta(seconds=x) for x in fh.variables['DMSP_COORDS_TIME'].data.ravel()]
...
for i, time in enumerate(sc_time):
    if time.minute % 10 == 0 and time.second == 0:
        ...
        if map_.llcrnrx < x < map_.urcrnrx and map_.llcrnry < y < map_.urcrnry:
            ...
            ax.text(x, y, 'DMSP F18'+'{:%H:%M}'.format(time), color='k', zorder=7, fontsize=4, fontweight='bold',
...
##NOAA-15
latSwarm,lonSwarm,timeSwarm=satorbit('./files/orbits/14_NOAA-15.txt')
...
for i, time in enumerate(timeSwarm):
    if time.minute % 10 == 0 and time.second == 0:
        ...
        if map_.llcrnrx < x < map_.urcrnrx and map_.llcrnry < y < map_.urcrnry:
            ...
            ax.text(x, y, 'NOAA-15 '+'{:%H:%M}'.format(time), color='k', zorder=7,fontsize=4, fontweight='bold',
                          path_effects=[pe.withStroke(foreground='w', linewidth=2)])
...
##NOAA-18
lat2,lon2,time2=satorbit('./files/orbits/14_NOAA-18.txt')
...
for i, time in enumerate(time2):
    if time.minute % 10 == 0 and time.second == 0:
        ...
        if map_.llcrnrx < x < map_.urcrnrx and map_.llcrnry < y < map_.urcrnry:
            ...
            ax.text(x, y, 'NOAA-18 '+'{:%H:%M}'.format(time), color='k', zorder=7, fontsize=4, fontweight='bold',
                          path_effects=[pe.withStroke(foreground='w', linewidth=2)])
...
#Add SuperDARN Scan
#===================================================
sTime=dt.datetime(2014, 12, 01, 16, 8)
...
plt.savefig('./rfe14_nosd.png', dpi=600)
